...
const { PageTimeoutError, SiteNotAnalyzableError } = require('../errors');
...
class PageLoader {
  ...
  _classifyError(err, url) {
    ...
    if (
      ...
      return new PageTimeoutError(url, this.timeoutMs);
    }
    ...
    return new SiteNotAnalyzableError(url, msg || 'unknown navigation error', err);
  }
  ...
  _friendlyReason(msg) {
    ...
    if (msg.includes('ERR_INTERNET_DISCONNECTED')) return 'no internet connection';
    if (msg.includes('ERR_ADDRESS_UNREACHABLE')) return 'address unreachable';
    if (msg.includes('ERR_CERT_') || msg.includes('ERR_SSL_')) return 'SSL/TLS certificate error';
